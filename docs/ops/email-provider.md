...
2. Set `EMAIL_PROVIDER`, `EMAIL_PROVIDER_DOMAIN`, `EMAIL_DKIM_SELECTOR`,
   ...
   `PRODUCTION_SMTP_PROBE_TO` in `.env.production`. The probe address must be
   a dedicated test mailbox and must not belong to a GradSync user account.
3. Run:

   ```bash
   docker compose -f docker-compose.prod.yml run --rm backend python manage.py check_production_readiness
   ```

4. Confirm the probe message is delivered and visible in provider logs.
5. Submit one periodic report and confirm the advisor notification delivery
   record reaches `sent`.
