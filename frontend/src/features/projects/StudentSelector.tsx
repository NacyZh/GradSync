...
export function StudentSelector({ onSelect, disabled = false, projectId, selectedIds = EMPTY_SELECTED_IDS }: StudentSelectorProps) {
  ...
  const showOptions = query.trim().length > 0 && data.length > 0;
  ...
  return (
    <div className="grid gap-2">
      <label className="grid gap-1.5 text-sm font-bold" htmlFor="student-selector">
        ...
        <span className="relative block">
          ...
          <Input
            ...
            autoComplete="off"
            aria-controls="student-selector-options"
      ...
      {showOptions ? (
        <ul id="student-selector-options" className="grid max-h-56 gap-1 overflow-auto rounded-md border bg-popover p-1 shadow-md" role="listbox" aria-label="Student search results">
          {data.map((student) => (
            <li key={student.id}>
              <button
                type="button"
