...
function mockUser(page: Parameters<typeof test>[1] extends (arg: infer T) => unknown ? T : never, role: string) {
  const user = { ...currentUser, global_role: role, name: `${role} User`, email: `${role}@example.edu` };
  return user;
}

test.describe('role workspaces', () => {
  test('admin sees account management and can navigate', async ({ page }) => {
    const adminUser = { id: 1, email: 'admin@gradsync.local', name: 'Admin User', global_role: 'admin', status: 'active' as const };
  ...
  test('advisor sees project management but no account admin', async ({ page }) => {
    const advisorUser = { id: 2, email: 'advisor@example.edu', name: 'Advisor User', global_role: 'advisor', status: 'active' as const };
  ...
  test('student cannot see project creation or account admin', async ({ page }) => {
    const studentUser = { id: 3, email: 'student@example.edu', name: 'Student User', global_role: 'student', status: 'active' as const };
