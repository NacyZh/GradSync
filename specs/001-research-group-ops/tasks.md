...
**Purpose**: Align the existing implementation with the updated production plan for configurable resources, local team-library imports, real-time locale switching, production login, validation seeding, and removal of legacy fixed/demo/upload semantics.

- [X] T227 CRITICAL: Replace fixed `LabResource`/`resource_id` resource implementation with configurable `ResourceType`, `ResourceItem`, `resource_item_id`, custom field validation, resource type/item endpoints, booking conflict updates, migrations, backend tests, and frontend resource booking updates in backend/apps/resources/*, backend/tests/**/*resource*, frontend/src/features/resources/*, frontend/tests/**/*resource*, and specs/001-research-group-ops/contracts/openapi.yaml per FR-011, FR-012, FR-013, US3/AC1..AC4, data-model.md, and plan: configurable resource schemas (contradicts)
- [X] T228 HIGH: Convert paper library implementation from DOI/file-style import to explicit local folder/file team-library import with `source_path_label`, local source types, relative paths, `imported_by`, no automatic external lookup, serializers/contracts/tests, and frontend import controls in backend/apps/library/*, backend/tests/**/*paper*, frontend/src/features/library/*, frontend/tests/**/*research-assets*, and specs/001-research-group-ops/contracts/openapi.yaml per FR-022, FR-023, FR-024, US4/AC1..AC2, data-model.md, and plan: local folder import (partial)
- [X] T229 HIGH: Convert code artifact implementation from upload semantics to local folder/archive import semantics with descriptions on versions, `source_path_label`, `relative_path_manifest`, `imported_by/imported_at`, `importCodeArtifactVersion` service/API naming, serializers/contracts/tests, and frontend code import controls in backend/apps/repositories/*, backend/tests/**/*code*, frontend/src/features/repositories/*, frontend/tests/**/*research-assets*, and specs/001-research-group-ops/contracts/openapi.yaml per FR-025, FR-026, US4/AC3, data-model.md, and plan: code team library local import (partial)
- [X] T230 HIGH: Replace production-facing `seed_demo_research_ops` usage and demo/sample seeded values with `seed_validation_research_ops`, production-shaped validation data, Docker Compose startup updates, README/quickstart-results updates, and test references in backend/apps/accounts/management/commands/*, backend/apps/projects/management/commands/*, docker-compose.yml, README.md, and specs/001-research-group-ops/quickstart-results.md per FR-030, FR-031, quickstart.md, and plan: production flows must not include prototype-only copy or behavior (contradicts)
